<?php
     ...
     *
     *  03/2024  project: authorizenet_cim v3.0.0 file: header_php.php
     */
...
// if the customer is not logged on, redirect them to the login page
    ...
    if ($addressSelected !== 'new') {
        $validAddress = $cim->isValidCustomerAddress($addressSelected);
    }
    ...
    if (isset($_POST['update_cid'])) {
        ...
        if (empty($payment_profile['payment_profile_id'])) {
            $messageStack->add_session(FILENAME_CARD_UPDATE,
                'There was a problem updating your card.  Please contact the store owner.', 'error');
            trigger_error('customer trying to update card not part of cust: ' . $customer_id . ' card_cid: ' . $_POST['update_cid']);
