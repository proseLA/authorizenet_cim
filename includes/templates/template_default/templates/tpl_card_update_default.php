...
if (isset($_GET['cid']) && ($_GET['action'] == 'delete')) {
    ...
    if ($valid_cid['valid']) {
        ...
        <div class='payment_select zebra'>
            Please confirm deletion of Credit Card ending in <span
                    class='last_four'><?php echo $valid_cid['last_four']; ?></span> expiring on
            <span class='expiration_date'>
		  <?php echo $valid_cid['exp_date']; ?>
		</span>
    ...
    if ($valid_cid['valid']) {
        ...
        <div class='review_box size18'>
            Update Credit Card ending in <span class='last_four'><?php echo $valid_cid['last_four']; ?></span>
        </div>
        <div class='review_box size18'>
            <?php
                echo 'Month:' . zen_draw_pull_down_menu('cc_month', $expires_month,
                    $def_month) . '&nbsp;Year:' . zen_draw_pull_down_menu('cc_year', $expires_year, $def_year);
            ?>
        </div>
        <?php
        require($template->get_template_dir('tpl_modules_address_book_details.php', DIR_WS_TEMPLATE, $current_page_base,
            'templates') . '/' . 'tpl_paul_cc_address.php');
        ?>
        <div id='update_cc'>
        ...
        <div class="new_card">
            <div class="fec-credit-card-info">
                <div class="fec-field">
                    ...
                    <?php echo zen_draw_input_field('cc_owner',
                ...
                <div class="fec-field">
                    ...
                    <?php echo zen_draw_pull_down_menu('cc_month', $expires_month,
                        $def_month) . '&nbsp;&nbsp;' . zen_draw_pull_down_menu('cc_year', $expires_year, $def_year); ?>
    ...
    <?php
        ...
        while (!$cards_saved->EOF) {
		  ...
		  <?php echo $card['exp_date']; ?>
                ...
                <span class='pay_buttons'>
		  <?php echo '<a href="' . zen_href_link(FILENAME_CARD_UPDATE, 'cid=' . $card['index_id'] . '&action=update',
              'SSL') . '"> '; ?>
		  <button class="a-button-text btn btn-primary">Edit</button></a>
		  <?php echo '<a href="' . zen_href_link(FILENAME_CARD_UPDATE, 'cid=' . $card['index_id'] . '&action=delete',
              'SSL') . '"> '; ?>
		  <button class="a-button-text btn btn-primary">Delete</button></a>
		</span>
                <?php
