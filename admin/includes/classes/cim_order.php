  ...
  function start() {
    ...
    if($this->purchase_order) {    // need a po before you can have po payments
      for($i = 0; $i < sizeof($this->purchase_order); $i++) {
        ...
        $po_payments_query = $db->Execute("select * from " . TABLE_CIM_PAYMENTS . "
...
//    $auto_refund = array();
      ...
      $pp_data = $db->Execute("select * from " . TABLE_PAYPAL . " where order_id = '" . $this->oID . "'");

// for each PayPal record, find a matching SO payment record. If can't find, create one
    while (!$pp_data->EOF) {
//        if ($pp_data->fields['mc_gross'] < 0) {  //refund
//            if ($pp_data->fields['payment_status'] == 'Refunded' || $pp_data->fields['payment_status'] == 'Reversed') {
//               fill in local array with info from query
//                $auto_refund['payment_id'] = 0; // $pp_data->fields['paypal_ipn_id'];  // must be payment_id of a payment or 0
//                $auto_refund['orders_id'] = $this->oID;
//                $auto_refund['refund_number'] = $pp_data->fields['paypal_ipn_id'] . ":" . $pp_data->fields['txn_id'];
//                $auto_refund['refund_name'] = $pp_data->fields['first_name'] . ' ' . $pp_data->fields['last_name'];
//                $auto_refund['refund_amount'] = -1.0 * $pp_data->fields['mc_gross'];
//                $auto_refund['refund_type'] = "PayPal:" . $pp_data->fields['payment_status'];
//                $auto_refund['date_posted'] = $pp_data->fields['payment_date'];

// check to see if this record already exists in SO_refunds
//                $so_data = $db->Execute("select * from " . TABLE_CIM_REFUNDS . " where refund_number = '" . $pp_data->fields['paypal_ipn_id'] . ":" . $pp_data->fields['txn_id'] . "'");
...
//                    zen_db_perform(TABLE_CIM_REFUNDS, $auto_refund);
//                }
//            } // payment_status check
//        } else { // payment, because mc_gross > 0
            if ($pp_data->fields['payment_status'] == 'Completed' || $pp_data->fields['payment_status'] == 'Canceled_Reversal') {
                $auto_payment['orders_id'] = $this->oID;
                $auto_payment['payment_number'] = $auto_payment['payment_number'] = $pp_data->fields['paypal_ipn_id'] . "-" . $pp_data->fields['txn_id'];
                $auto_payment['payment_name'] = $pp_data->fields['first_name'] . ' ' . $pp_data->fields['last_name'];
                $auto_payment['payment_amount'] = $pp_data->fields['mc_gross'];
                $auto_payment['payment_type'] = $pp_data->fields['module_mode'];
                $auto_payment['date_posted'] = $pp_data->fields['payment_date'];
                $auto_payment['last_modified'] = $pp_data->fields['payment_date'];
                //$auto_payment['purchase_order_id'] = 0;    // $pp_data->fields['txn_id'];

                // check to see if this record already exists in SO_Payments
                $so_data = $db->Execute("select * from " . TABLE_CIM_PAYMENTS . " where orders_id = '" . $this->oID . "'");
                // if not yet recorded, enter into the table
                if ($so_data->EOF) {
				zen_db_perform(TABLE_CIM_PAYMENTS, $auto_payment);
                }
            } // payment_status check
...
// for each Credit Card paid order, find a matching SO payment record. If can't find, create one
    while (!$ccpay_data->EOF) {
            if ($ccpay_data->fields['payment_module_code'] == 'authorizenet_aim' || $ccpay_data->fields['payment_method'] == 'Credit Card') {
                ...
                // if not yet recorded, enter into the table
                if ($so_data->EOF) {
				//zen_db_perform(TABLE_CIM_PAYMENTS, $auto_payment);  WHAT BULLSHIT!
                }
            } // payment_status check
       $ccpay_data->MoveNext();
